    ...
    <SectionWrapper id="proposals">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {proposals.map((proposal) => {
          const iconPath = iconPaths[proposal.icon ?? 'default'] ?? iconPaths.default;
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div className="absolute left-3 top-3 flex h-10 w-10 items-center justify-center rounded-xl bg-accent/10 text-accent">
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d={iconPath} />
                  </svg>
                </div>
                <div className="absolute right-3 top-3">
                  <span className={cn('inline-flex rounded-full border px-3 py-1 text-xs font-semibold', badge.className)}>
                    {badge.label}
                  </span>
                </div>
              </div>
              <div className="p-5">
                <h3 className="mb-2 font-bold text-on-primary">{proposal.title}</h3>
                <p className="text-sm leading-relaxed text-muted">{proposal.description}</p>
              </div>
            </div>
          );
        })}
